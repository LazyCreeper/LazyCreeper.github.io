...
//MCSM-HELP
app.use('/mcsm-help', express.static('./mcsm-help'));

app.use('/core', express.static('./core'));
app.use('/ftpd', express.static('./ftpd'));
app.use('/helper', express.static('./helper'));
app.use('/model', express.static('./model'));
app.use('/onlinefs', express.static('./onlinefs'));
...
MCSERVER.infoLog('Lazy', 'Welcome use Mcserver Manager!');
MCSERVER.infoLog('INFO', '嘤~嘤~嘤~~~(划掉)');
...
(function initializationProm() {
    ...
    app.listen(MCSERVER.localProperty.http_port, MCSERVER.localProperty.http_ip, () => {
        ...
        MCSERVER.infoLog('INFO', '加载配置文件: property.js ');

        if (MCSERVER.allError <= 0) {
            MCSERVER.infoLog('INFO', '控制面板已经启动');
			MCSERVER.infoLog('INFO', '如需关闭面板，按 Ctrl + c 即可'.red );
        } else {
...
app.get('*', function (req, res) {
    //404 页面
    res.redirect('/template/Error/404.html');
    res.end();
})
